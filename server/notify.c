...
void notify_parent_register(pid_t pid, int pipe_read_fd, int client_fd) {
    if (g_parent_count >= MAX_CLIENTS) return;
...
void notify_user(const char *login, Notification notif) {
    if (g_child_pipe_write < 0) return;
...
void notify_dispatch(void) {
    ...
    for (int i = 0; i < g_parent_count; i++) {
        ...
        char hdr[PIPE_HDR_SIZE];
        if (read(pfd, hdr, PIPE_HDR_SIZE) != (ssize_t)PIPE_HDR_SIZE) continue;
        ...
        if (json_len == 0 || json_len > MSG_BUFFER_SIZE) continue;
