...
make clean  # удалить объектники и бинарники
```

Исполняемые файлы появляются в `bin/`: `bin/server`, `bin/client`.
Объектные файлы — в `bin/obj/`.
...
```bash
./bin/server          # запуск на порту 8080 (по умолчанию)
./bin/server 9000     # запуск на произвольном порту
```

Перед первым запуском убедитесь, что папка `data/` существует:

```bash
mkdir -p data/messages/chat data/messages/private
```
...
При запуске программа последовательно спрашивает:

1. Адрес сервера в формате `host:port` (например `127.0.0.1:8080`)
2. Действие: `1` — войти, `2` — зарегистрироваться
3. Логин и пароль

После успешного входа открывается основной интерфейс.
...
Интерфейс терминальный, реализован на ncurses. После запуска приложения пользователю предлагается ввести адрес сервера, логин и пароль. Также доступна регистрация нового аккаунта. После успешного входа открывается основной интерфейс.

### Основной экран

Экран состоит из трёх зон:

- **Левый блок** — активный чат: сверху список сообщений, снизу поле ввода нового сообщения.
- **Правый блок** — список чатов или пользователей (два режима).
- **Системная строка** — нижняя полоса экрана: отображает системные сообщения и принимает командный ввод.

### Правый блок: режимы отображения

Правый блок работает в двух режимах, переключаемых клавишами `←` / `→` при активном правом блоке:

| Режим | Описание |
| :--- | :--- |
| **Chats** | Список групповых чатов, в которых состоит текущий пользователь |
| **Users** | Список всех пользователей сервера. Пользователи с которыми есть переписка — выделены (A_BOLD при выборе), без переписки — приглушены (A_DIM) |

### Навигация

| Ситуация | Действие | Клавиша |
| :--- | :--- | :--- |
| Ни один блок не активен | Переключение лево/право | `←` / `→` |
| Ни один блок не активен | Перейти к системной строке | `↓` |
| Ни один блок не активен | Войти в выбранный блок | `Enter` |
| Блок с чатом активен | Прокрутка истории сообщений | `↑` / `↓` |
| Правый блок активен | Выбор элемента | `↑` / `↓` |
| Правый блок активен | Переключить режим (чаты/пользователи) | `←` / `→` |
| Правый блок активен | Открыть выбранный чат или диалог | `Enter` |
| Любой активный блок | Выйти из блока | `Escape` |

### Цвет рамки блока

| Состояние | Цвет рамки |
| :--- | :--- |
| Блок не выбран и не активен | Белый |
| Блок выбран (в фокусе, но не активен) | Жёлтый |
| Блок активен (ввод / прокрутка) | Зелёный |

### Системная строка

Нижняя строка экрана выполняет две функции:
1. **Вывод статуса** — отображает результат последнего действия (например, «Creating chat 'dev'...»).
2. **Командный ввод** — при активации (`↓` → `Enter`) позволяет вводить команды.

Команды вводятся как в системной строке, так и прямо в поле ввода чата (строка начинается с `/`):

| Команда | Описание |
| :--- | :--- |
| `/create <chatname>` | Создать новый групповой чат |
| `/add <username>` | Добавить пользователя в текущий чат |
| `/delete <username>` | Удалить пользователя из текущего чата (только для создателя) |
...
### `client/ui.h` / `ui.c`
Отображение интерфейса в терминале (ncurses) и обработка ввода.
- `void ui_init()` — инициализация ncurses, создание окон
- `void ui_destroy()` — завершение ncurses, освобождение окон
- `int ui_prompt_server(char *host_out, int *port_out)` — ввод адреса сервера
- `int ui_prompt_login(char *login_out, char *password_out)` — ввод логина и пароля
- `int ui_prompt_register(char *login_out, char *password_out)` — ввод данных регистрации
- `void ui_run()` — главный цикл интерфейса: навигация между блоками, обработка клавиш
- `void ui_set_chat(const char *chat_name, Message *msgs, int count)` — установить активный чат и его сообщения в левый блок
- `void ui_set_chat_list(char **names, int count)` — обновить список чатов в правом блоке
- `void ui_set_user_list(char **names, int *has_messages, int count)` — обновить список пользователей; `has_messages[i]` = 1 если есть переписка
- `void ui_append_message(Message *msg)` — добавить новое сообщение в левый блок без полной перерисовки
- `void ui_notify(const char *text)` — показать всплывающее уведомление пользователю
- `void ui_sys(const char *text)` — вывести системное сообщение в нижнюю строку
...
- `void *action_add_chat_user(void *args)` — добавить пользователя в чат
- `void *action_remove_chat_user(void *args)` — удалить пользователя из чата
- `void *action_leave_chat(void *args)` — покинуть чат
